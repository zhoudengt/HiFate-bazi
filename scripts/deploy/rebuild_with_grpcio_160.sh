#!/bin/bash
# 在服务器上重建Docker镜像（使用grpcio 1.60.0）
# 使用方式：bash scripts/deploy/rebuild_with_grpcio_160.sh [node1|node2|both]
...
echo "=========================================="
echo "   重建Docker镜像（grpcio 1.60.0）"
echo "=========================================="
echo ""

if [ "$NODE" = "node1" ] || [ "$NODE" = "both" ]; then
    echo "🔨 在Node1上拉取最新代码并重建镜像..."
...
if [ "$NODE" = "node1" ] || [ "$NODE" = "both" ]; then
    echo "Node1依赖版本："
    ssh_exec $NODE1_IP "docker exec hifate-web python -c 'import grpc, protobuf; print(\"✅ grpc:\", grpc.__version__); print(\"✅ protobuf:\", protobuf.__version__)'" || echo "❌ 无法验证"
fi
if [ "$NODE" = "node2" ] || [ "$NODE" = "both" ]; then
    echo "Node2依赖版本："
    ssh_exec $NODE2_IP "docker exec hifate-web python -c 'import grpc, protobuf; print(\"✅ grpc:\", grpc.__version__); print(\"✅ protobuf:\", protobuf.__version__)'" || echo "❌ 无法验证"
fi
