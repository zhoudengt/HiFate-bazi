...
ENVIRONMENTS = {
    ...
    "node2": "http://47.243.160.43:8001",  # Node2 公网（与 Node1 一致）
    "node2-local": "http://localhost:8001",  # Node2 宿主机
    "node2-docker": "http://localhost:8001"  # Node2 容器内（docker exec 用）
}
...
def main():
    ...
    parser.add_argument("--env", choices=["local", "production", "node2", "node2-local", "node2-docker"], default="local",
                        help="测试环境 (默认: local)")
