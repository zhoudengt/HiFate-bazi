...
def add_wuxing_proportion_stream_doc(doc):
    ...
    interface_details = [
        ...
        ('备注', '与 /bazi/wuxing-proportion 接口相同的输入，但以SSE流式方式返回数据：1. 首先返回完整的五行占比数据（type: "data"）2. 然后流式返回大模型分析（type: "progress"）3. 最后返回完成标记（type: "complete"）。返回五行占比统计、四柱十神信息、旺衰分析结果、相生相克关系等'),
    ]
    ...
    response_fields = [
        ('success', 'bool', '是否成功'),
        ('data', 'dict', '五行占比数据'),
        ('  data.wuxing_proportion', 'dict', '五行占比统计'),
        ('    wuxing_proportion.金', 'int', '金的数量'),
        ('    wuxing_proportion.木', 'int', '木的数量'),
        ('    wuxing_proportion.水', 'int', '水的数量'),
        ('    wuxing_proportion.火', 'int', '火的数量'),
        ('    wuxing_proportion.土', 'int', '土的数量'),
        ('  data.shishen_info', 'dict', '四柱十神信息'),
        ('    shishen_info.year', 'dict', '年柱十神（主星、副星）'),
        ('    shishen_info.month', 'dict', '月柱十神（主星、副星）'),
        ('    shishen_info.day', 'dict', '日柱十神'),
        ('    shishen_info.hour', 'dict', '时柱十神（主星、副星）'),
        ('  data.wangshuai', 'dict', '旺衰分析结果'),
        ('    wangshuai.status', 'str', '旺衰状态'),
        ('    wangshuai.score', 'int', '旺衰分数'),
        ('  data.xiangsheng_xiangke', 'dict', '相生相克关系'),
...
def add_xishen_jishen_stream_doc(doc):
    ...
    interface_details = [
        ...
        ('备注', '与 /bazi/xishen-jishen 接口相同的输入，但以SSE流式方式返回数据：1. 首先返回完整的喜神忌神数据（type: "data"）2. 然后流式返回大模型分析（type: "progress"）3. 最后返回完成标记（type: "complete"）。返回喜神五行、忌神五行、十神命格、旺衰状态等'),
    ]
...
def main():
    ...
    # 保存文档
    doc.save(doc_path)
    print(f'✅ 两个流式接口文档已成功追加到 {doc_path}')
