...
class BaziCoreClientAdapter(IBaziCoreClient):
    ...
    def __init__(self, base_url: str, timeout: float = 30.0):
        ...
        if self._enable_circuit_breaker:
            try:
                ...
                self._circuit_breaker = CircuitBreaker.get("bazi-core", config=config)
                logger.info("熔断器已启用: bazi-core")
    ...
    def _call_with_protection(self, solar_date: str, solar_time: str, gender: str) -> Dict[str, Any]:
            ...
            if not self._circuit_breaker.allow_request():
                ...
                if self._enable_metrics and self._metrics_collector:
                    ...
                    self._metrics_collector.record_grpc_call(
                        "bazi-core", "calculate_bazi", 
                        success=False, duration=duration, error_type=error_type
                    )
                raise CircuitBreakerOpen("bazi-core", "Circuit breaker is open")
        ...
        for attempt in range(max_retries):
            try:
                result = self._client.calculate_bazi(solar_date, solar_time, gender)
                ...
                # 等待后重试
                logger.warning(
                    f"bazi-core 调用失败（尝试 {attempt + 1}/{max_retries}）: {e}，"
                    f"{retry_delay} 秒后重试..."
                )
