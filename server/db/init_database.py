...
def init_database():
    ...
    try:
                ...
                with conn.cursor() as cursor:
                    for statement in statements:
                        if statement:
                            try:
                                cursor.execute(statement)
                                logger.info(f"  ✓ 迁移执行成功: {statement[:80]}...")
                            except Exception as e:
                                # 列已存在等错误忽略
                                if "Duplicate" in str(e) or "already exists" in str(e) or "errno: 1060" in str(e):
                                    logger.info(f"  • 跳过（已存在）: {statement[:80]}...")
                                else:
                                    logger.info(f"  ⚠ 迁移警告: {e}")
                    conn.commit()
        ...
        if result:
            logger.info(f"  ✓ 当前数据库: {result[0]['db']}")
        
        result = connector.execute_query("SHOW TABLES")
        if result:
            logger.info(f"  ✓ 表数量: {len(result)}")
            for table in result:
