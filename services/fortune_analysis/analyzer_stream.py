...
from analyzer import FortuneAnalyzer
    ...
    async def analyze_hand_stream(
        ...
        try:
            ...
            try:
                ...
                # 计算置信度
                try:
                    confidence = self.analyzer._calculate_confidence(hand_features, len(all_insights))
    ...
    async def analyze_face_stream(
        ...
        try:
            ...
            try:
                from image_validator import ImageValidator
                validator = ImageValidator()
                is_valid, error_msg = validator.validate_face_image(image_bytes)
                if not is_valid:
                    yield {
                        "type": "error",
                        "error": error_msg,
            ...
            face_result = await loop.run_in_executor(
                None,
                lambda: self.face_analyzer.analyze(image_bytes, image_format, enable_special_features=False)
            )
            
            if not face_result.get("success"):
                yield {
                    "type": "error",
                    "error": face_result.get("error", "面相分析失败"),
                    "statusText": "分析失败"
                }
                return
            ...
            rule_result = await loop.run_in_executor(
                None,
                self.rule_client.match_face_rules,
                face_features,
                bazi_info_dict,
                None  # 让微服务内部获取八字数据
            )
            ...
            try:
                ...
                # 计算置信度
                try:
                    confidence = self.analyzer._calculate_confidence(face_features, len(all_insights))
                except Exception as conf_error:
                    print(f"⚠️  计算置信度失败: {conf_error}，使用默认值")
                    confidence = 0.7  # 默认置信度
                ...
                try:
                    fallback_report = {
                        "success": True,
                        "features": face_features,
                        "insights": face_insights + integrated_insights,
                        "integrated_insights": integrated_insights,
                        "bazi_data": bazi_data,
                        "recommendations": recommendations,
                        "confidence": 0.7,
