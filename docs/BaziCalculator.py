...
class BaziCalculator:
    ...
    def calculate_dayun_liunian(self, current_time=None):
        """
        ...
        # 计算流月序列
        self._calculate_liuyue_sequence()
    ...
    def _calculate_dayun_sequence(self):
        ...
        for i in range(10):
            ...
            if dayun_direction == '正排':
                stem_index = (month_stem_index + i + 1) % 10  # 月柱+1是第一步，月柱+2是第二步
            ...
            main_star = self.get_main_star(day_stem, stem, 'dayun')

            # 动态计算年龄和年份
            qiyun_age_int = int(qiyun_total_age)

            if i == 0:
                # 第一步：根据起运年龄决定
                if qiyun_total_age <= 1:
                    # 起运年龄小于等于1岁，直接进入第一步大运，年龄显示为1岁
                    age_display = "1岁"
                    year_start = birth_year
                    year_end = birth_year + 9
                else:
                    # 起运年龄大于1岁，有"小运"期，年龄显示范围
                    age_display = f"1~{qiyun_age_int}岁"
                    year_start = birth_year
                    year_end = birth_year + qiyun_age_int  # 到起运年
            else:
                # 后续大运：从起运年开始，每10年一步
                if qiyun_total_age <= 1:
                    # 起运年龄小于等于1岁
                    # 第二步大运从出生年后10年开始，年龄从11岁开始
                    age_start = 1 + i * 10
                    year_start = birth_year + i * 10
                else:
                    # 起运年龄大于1岁
                    # 第二步大运从起运年龄+10开始
                    age_start = qiyun_age_int + (i - 1) * 10 + 1
                    year_start = birth_year + qiyun_age_int + (i - 1) * 10

                age_display = f"{age_start}岁"
                # year_end 应该是 inclusive，表示该年结束
                # 大运10年，从year_start开始，到year_start+10年结束（包含起止年份）
                # 所以 year_end = year_start + 10
    ...
    def _calculate_liunian_sequence(self):
        ...
        # 计算前后5年的流年
        for year_offset in range(-5, 6):
            target_year = self.current_time.year + year_offset

            # 使用lunar-python计算该年的干支
            solar = Solar.fromYmdHms(target_year, 1, 1, 12, 0, 0)
            lunar = solar.getLunar()
            bazi = lunar.getBaZi()

            liunian_stem = bazi[0][0]
            liunian_branch = bazi[0][1]
            main_star = self.get_main_star(day_stem, liunian_stem, 'liunian')
        ...
        self.details['liunian_sequence'] = liunian_sequence

        print("流年序列:")
        years_line = "  ".join([f"{item['year']}" for item in liunian_sequence])
        stems_line = "  ".join([f"{item['stem']}{item['main_star']}" for item in liunian_sequence])
        branches_line = "  ".join([f"{item['branch']}" for item in liunian_sequence])
