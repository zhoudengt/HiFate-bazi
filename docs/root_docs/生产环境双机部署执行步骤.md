# 生产环境双机部署执行步骤

## 📋 环境信息

- **Node1**: 公网 `8.210.52.217` / 私网 `172.18.121.222`
- **Node2**: 公网 `47.243.160.43` / 私网 `172.18.121.223`
- **MySQL 密码**: `${SSH_PASSWORD}`
- **Git 仓库**: `https://github.com/zhoudengt/HiFate-bazi`
- **SECRET_KEY**: `kx9078L34ZoROnneJu8fMmJ70JImvVan88JYvxiewbE`

## ⚠️ 数据库连接规范

**重要说明**：
- **所有环境统一连接生产Node1 Docker MySQL和MongoDB**：生产、开发环境都连接生产Node1 Docker数据库，确保数据一致性
- **代码开发流程不变**：代码开发必须遵循正规流程：本地开发 → Git提交 → 增量部署到双机
- **禁止直接在生产服务器开发**：严禁在生产服务器上直接修改代码，必须通过Git版本控制和增量部署

**数据库连接信息**：
- **MySQL**: `8.210.52.217:3306`（本地开发使用公网IP），`172.18.121.222:3306`（生产环境使用内网IP）
- **MongoDB**: `8.210.52.217:27017`（本地开发使用公网IP），`172.18.121.222:27017`（生产环境使用内网IP）

---

## 🚀 部署步骤

### 步骤 1: 部署 Node1（主节点）

**1.1 登录 Node1**
```bash
ssh root@8.210.52.217
```

**1.2 上传部署脚本**
```bash
# 在本地执行（从项目目录）
scp deploy/scripts/deploy_production_manual.sh root@8.210.52.217:/root/
scp deploy/scripts/init-ecs.sh root@8.210.52.217:/root/
scp deploy/scripts/deploy.sh root@8.210.52.217:/root/
```

**1.3 在 Node1 上执行部署**
```bash
# 在 Node1 上执行
cd /root
bash deploy_production_manual.sh
# 当提示节点类型时，输入：node1
```

**或者手动执行**：
```bash
# 1. 初始化（安装 Docker）
bash init-ecs.sh

# 2. 克隆代码
mkdir -p /opt/HiFate-bazi
cd /opt/HiFate-bazi
git clone https://github.com/zhoudengt/HiFate-bazi .

# 3. 配置环境变量
cat > .env << 'EOF'
NODE_ID=node1
NODE1_IP=172.18.121.222
NODE2_IP=172.18.121.223
ACR_REGISTRY=registry.cn-hangzhou.aliyuncs.com
ACR_NAMESPACE=hifate
ACR_USERNAME=${ACR_USERNAME:-${ACR_ACCESS_KEY_ID}}  # 从环境变量读取
ACR_PASSWORD=${ACR_PASSWORD:-${ACR_ACCESS_KEY_SECRET}}  # 从环境变量读取
IMAGE_TAG=latest
MYSQL_USER=root
MYSQL_PASSWORD=${SSH_PASSWORD}
MYSQL_DATABASE=hifate_bazi
MYSQL_REPL_PASSWORD=${SSH_PASSWORD}
REDIS_PASSWORD=
SECRET_KEY=kx9078L34ZoROnneJu8fMmJ70JImvVan88JYvxiewbE
EOF

# 4. 部署
bash deploy/scripts/deploy.sh node1
```

**1.4 配置 MySQL 主从复制用户**
```bash
docker exec -it hifate-mysql-master mysql -uroot -p${SSH_PASSWORD}

# 在 MySQL 中执行：
CREATE USER 'repl'@'%' IDENTIFIED BY '${SSH_PASSWORD}';
GRANT REPLICATION SLAVE ON *.* TO 'repl'@'%';
FLUSH PRIVILEGES;
SHOW MASTER STATUS;
exit;
```

---

### 步骤 2: 部署 Node2（从节点）

**2.1 登录 Node2**
```bash
ssh root@47.243.160.43
```

**2.2 上传部署脚本**
```bash
# 在本地执行（从项目目录）
scp deploy/scripts/deploy_production_manual.sh root@47.243.160.43:/root/
scp deploy/scripts/init-ecs.sh root@47.243.160.43:/root/
scp deploy/scripts/deploy.sh root@47.243.160.43:/root/
```

**2.3 在 Node2 上执行部署**
```bash
# 在 Node2 上执行
cd /root
bash deploy_production_manual.sh
# 当提示节点类型时，输入：node2
```

**或者手动执行**：
```bash
# 1. 初始化（安装 Docker）
bash init-ecs.sh

# 2. 克隆代码
mkdir -p /opt/HiFate-bazi
cd /opt/HiFate-bazi
git clone https://github.com/zhoudengt/HiFate-bazi .

# 3. 配置环境变量
cat > .env << 'EOF'
NODE_ID=node2
NODE1_IP=172.18.121.222
NODE2_IP=172.18.121.223
ACR_REGISTRY=registry.cn-hangzhou.aliyuncs.com
ACR_NAMESPACE=hifate
ACR_USERNAME=${ACR_USERNAME:-${ACR_ACCESS_KEY_ID}}  # 从环境变量读取
ACR_PASSWORD=${ACR_PASSWORD:-${ACR_ACCESS_KEY_SECRET}}  # 从环境变量读取
IMAGE_TAG=latest
MYSQL_USER=root
MYSQL_PASSWORD=${SSH_PASSWORD}
MYSQL_DATABASE=hifate_bazi
MYSQL_REPL_PASSWORD=${SSH_PASSWORD}
REDIS_PASSWORD=
SECRET_KEY=kx9078L34ZoROnneJu8fMmJ70JImvVan88JYvxiewbE
EOF

# 4. 部署
bash deploy/scripts/deploy.sh node2
```

**2.4 配置 MySQL 从库**
```bash
docker exec -it hifate-mysql-slave mysql -uroot -p${SSH_PASSWORD}

# 在 MySQL 中执行：
CHANGE MASTER TO
    MASTER_HOST='172.18.121.222',
    MASTER_USER='repl',
    MASTER_PASSWORD='${SSH_PASSWORD}',
    MASTER_AUTO_POSITION=1;
START SLAVE;
SHOW SLAVE STATUS\G
# 确认 Slave_IO_Running 和 Slave_SQL_Running 都是 Yes
exit;
```

---

### 步骤 3: 验证部署

**3.1 检查服务状态**
```bash
# Node1
ssh root@8.210.52.217 "docker ps | grep hifate"

# Node2
ssh root@47.243.160.43 "docker ps | grep hifate"
```

**3.2 检查健康状态**
```bash
# Node1
curl http://8.210.52.217/health
curl http://8.210.52.217:8001/health

# Node2
curl http://47.243.160.43/health
curl http://47.243.160.43:8001/health
```

**3.3 检查 MySQL 主从复制**
```bash
ssh root@47.243.160.43 "docker exec -i hifate-mysql-slave mysql -uroot -p${SSH_PASSWORD} -e \"SHOW SLAVE STATUS\\G\" | grep -E 'Slave_IO_Running|Slave_SQL_Running'"
```

**3.4 检查 Redis 主从复制**
```bash
# Node1（主库）
ssh root@8.210.52.217 "docker exec -it hifate-redis-master redis-cli INFO replication"

# Node2（从库）
ssh root@47.243.160.43 "docker exec -it hifate-redis-slave redis-cli INFO replication"
```

---

## ⚠️ 注意事项

1. **不要操作测试环境**：所有操作都在生产环境的 Node1 和 Node2 上，不要影响测试环境 `123.57.216.15`
2. **安全组配置**：确保两台机器之间可以互相访问（内网互通）
3. **端口开放**：确保安全组开放了 80, 443, 22, 3306, 6379 端口
4. **MySQL 主从复制**：确保 Node2 可以访问 Node1 的 MySQL（内网 IP: 3306）
5. **Redis 主从复制**：确保 Node2 可以访问 Node1 的 Redis（内网 IP: 6379）

---

## 🔍 故障排查

### 服务无法启动
```bash
# 查看服务日志
docker logs hifate-web
docker logs hifate-mysql-master
docker logs hifate-redis-master

# 查看所有服务状态
docker ps -a
```

### MySQL 主从复制失败
```bash
# 检查从库状态
docker exec -it hifate-mysql-slave mysql -uroot -p${SSH_PASSWORD} -e "SHOW SLAVE STATUS\G"

# 检查网络连接
docker exec -it hifate-mysql-slave ping 172.18.121.222
```

### 镜像拉取失败
```bash
# 手动登录 ACR
docker login registry.cn-hangzhou.aliyuncs.com -u ${ACR_USERNAME:-${ACR_ACCESS_KEY_ID}}
# 输入密码：${ACR_PASSWORD:-${ACR_ACCESS_KEY_SECRET}}

# 手动拉取镜像
docker pull registry.cn-hangzhou.aliyuncs.com/hifate/hifate-bazi:latest
```

---

**最后更新**：2025-01-21

