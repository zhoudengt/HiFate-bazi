# 生产环境架构和部署规范

## 📋 文档说明

本文档记录生产环境的架构信息、部署配置和运维规范，确保系统的继承性、稳定性和可用性。

**最后更新**：2025-12-12

---

## 🏗️ 生产环境架构

### 双机部署架构

```
┌─────────────────────────────────────────────────────────────────┐
│                        生产环境双机架构                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌──────────────────────┐         ┌──────────────────────┐    │
│  │      Node1 (主节点)   │         │      Node2 (从节点)   │    │
│  │  8.210.52.217        │         │  47.243.160.43       │    │
│  │  172.18.121.222      │         │  172.18.121.223      │    │
│  ├──────────────────────┤         ├──────────────────────┤    │
│  │  Nginx (80/443)      │◄───────►│  Nginx (80/443)      │    │
│  │  Web (8001)          │         │  Web (8001)          │    │
│  │  MySQL 主库 (3306)   │◄──复制──►│  MySQL 从库 (3306)   │    │
│  │  Redis 主库 (6379)   │◄──复制──►│  Redis 从库 (6379)   │    │
│  │  微服务 (9001-9010)  │         │  微服务 (9001-9010)  │    │
│  └──────────────────────┘         └──────────────────────┘    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 服务器信息

| 节点 | 公网IP | 内网IP | 角色 | 状态 |
|------|--------|--------|------|------|
| Node1 | 8.210.52.217 | 172.18.121.222 | 主节点（MySQL主/Redis主） | ✅ 运行中 |
| Node2 | 47.243.160.43 | 172.18.121.223 | 从节点（MySQL从/Redis从） | ✅ 运行中 |
| 测试环境 | 123.57.216.15 | - | 测试环境 | ✅ 运行中 |

### 服务端口清单

| 服务 | 端口 | 说明 |
|------|------|------|
| Nginx | 80, 443 | 负载均衡和反向代理 |
| Web 服务 | 8001 | FastAPI 主服务 |
| bazi-core | 9001 | 八字核心计算服务 |
| bazi-fortune | 9002 | 运势计算服务 |
| bazi-analyzer | 9003 | 八字分析服务 |
| bazi-rule | 9004 | 规则匹配服务 |
| fortune-analysis | 9005 | 运势分析服务 |
| payment-service | 9006 | 支付服务 |
| fortune-rule | 9007 | 运势规则服务 |
| intent-service | 9008 | 意图识别服务 |
| prompt-optimizer | 9009 | 提示优化服务 |
| desk-fengshui | 9010 | 办公桌风水分析服务 |
| MySQL | 3306 | 数据库 |
| Redis | 6379 | 缓存 |

---

## 🔧 负载均衡配置

### Nginx 负载均衡

**配置位置**：`deploy/nginx/conf.d/hifate.conf`

**上游服务器配置**：
```nginx
upstream web_backend {
    # Node1 应用服务（内网IP）
    server 172.18.121.222:8001 weight=1 max_fails=3 fail_timeout=30s;
    # Node2 应用服务（内网IP）
    server 172.18.121.223:8001 weight=1 max_fails=3 fail_timeout=30s;
    
    keepalive 32;
}
```

**关键配置**：
- ✅ **必须使用内网 IP**（172.18.121.222/223）
- ❌ **禁止使用公网 IP**（8.210.52.217/47.243.160.43）
- ❌ **禁止使用占位符**（NODE1_IP/NODE2_IP）

**超时配置**（已优化）：
```nginx
# 后端 API
location /api/ {
    proxy_pass http://web_backend;
    
    # 优化超时设置：快速失败，提升用户体验
    proxy_connect_timeout 10s;   # 连接超时：10秒（快速检测服务不可用）
    proxy_send_timeout 30s;      # 发送超时：30秒（适合大多数API）
    proxy_read_timeout 30s;      # 读取超时：30秒（适合大多数API）
    
    # 故障转移：快速切换到备用节点
    proxy_next_upstream error timeout http_500 http_502 http_503 http_504;
    proxy_next_upstream_tries 2;  # 最多尝试2个节点
    proxy_next_upstream_timeout 5s;  # 故障转移超时：5秒（快速切换）
}
```

**故障转移机制**：
- `max_fails=3`：连续失败3次后标记为不可用
- `fail_timeout=30s`：30秒后重新尝试
- `proxy_next_upstream`：自动切换到备用节点

---

## 💾 数据库主从复制

### MySQL 主从复制

**主库配置**（Node1）：
- **地址**：172.18.121.222:3306
- **容器名**：`hifate-mysql-master`
- **配置文件**：`deploy/mysql/master.cnf`
- **GTID 模式**：已启用

**从库配置**（Node2）：
- **地址**：172.18.121.223:3306
- **容器名**：`hifate-mysql-slave`
- **配置文件**：`deploy/mysql/slave.cnf`
- **只读模式**：已启用

**复制用户**：
```sql
CREATE USER 'repl'@'%' IDENTIFIED BY 'Yuanqizhan@163';
GRANT REPLICATION SLAVE ON *.* TO 'repl'@'%';
FLUSH PRIVILEGES;
```

**从库配置命令**：
```sql
CHANGE MASTER TO
    MASTER_HOST='172.18.121.222',
    MASTER_USER='repl',
    MASTER_PASSWORD='Yuanqizhan@163',
    MASTER_AUTO_POSITION=1;
START SLAVE;
```

**状态检查**：
```sql
SHOW SLAVE STATUS\G
-- 关键指标：
-- Slave_IO_Running: Yes
-- Slave_SQL_Running: Yes
-- Seconds_Behind_Master: 0
```

### Redis 主从复制

**主库配置**（Node1）：
- **地址**：172.18.121.222:6379
- **容器名**：`hifate-redis-master`
- **配置文件**：`deploy/redis/master.conf`

**从库配置**（Node2）：
- **地址**：172.18.121.223:6379
- **容器名**：`hifate-redis-slave`
- **配置文件**：`deploy/redis/slave.conf`
- **主库地址**：172.18.121.222:6379

---

## 🔐 部署配置信息

### 环境变量

**数据库配置**：
- **MySQL 密码**：`Yuanqizhan@163`
- **MySQL 复制密码**：`Yuanqizhan@163`
- **MySQL 数据库**：`hifate_bazi`

**应用配置**：
- **SECRET_KEY**：从环境变量 `SECRET_KEY` 读取（生产环境使用强随机密钥）
- **ACR 用户名**：从环境变量 `ACR_USERNAME` 或 `ACR_ACCESS_KEY_ID` 读取
- **ACR 密码**：从环境变量 `ACR_PASSWORD` 或 `ACR_ACCESS_KEY_SECRET` 读取

**Git 仓库**：
- **仓库地址**：`https://github.com/zhoudengt/HiFate-bazi`
- **默认分支**：`master`

### 项目目录

| 路径 | 说明 |
|------|------|
| `/opt/HiFate-bazi` | 项目根目录 |
| `/opt/HiFate-bazi/.env` | 环境变量文件 |
| `/opt/HiFate-bazi/deploy/docker` | Docker Compose 配置 |
| `/opt/HiFate-bazi/logs` | 日志目录 |

---

## 🚀 部署流程

### 首次部署

1. **初始化服务器**：
   ```bash
   bash deploy/scripts/init-ecs.sh
   ```

2. **克隆代码**：
   ```bash
   cd /opt/HiFate-bazi
   git clone https://github.com/zhoudengt/HiFate-bazi .
   ```

3. **配置环境变量**：
   ```bash
   cp deploy/env/env.template .env
   vim .env  # 编辑配置
   ```

4. **修复 gRPC 代码**（必须）：
   ```bash
   python3 scripts/grpc/fix_version_check.py
   ```

5. **配置 Nginx**（必须）：
   ```bash
   # 替换 IP 占位符
   sed -i 's/NODE1_IP/172.18.121.222/g' deploy/nginx/conf.d/hifate.conf
   sed -i 's/NODE2_IP/172.18.121.223/g' deploy/nginx/conf.d/hifate.conf
   ```

6. **部署 Node1**：
   ```bash
   bash deploy/scripts/deploy.sh node1
   ```

7. **配置 MySQL 主从复制用户**（Node1）：
   ```sql
   CREATE USER 'repl'@'%' IDENTIFIED BY 'Yuanqizhan@163';
   GRANT REPLICATION SLAVE ON *.* TO 'repl'@'%';
   FLUSH PRIVILEGES;
   ```

8. **部署 Node2**：
   ```bash
   bash deploy/scripts/deploy.sh node2
   ```

9. **配置 MySQL 从库**（Node2）：
   ```sql
   CHANGE MASTER TO
       MASTER_HOST='172.18.121.222',
       MASTER_USER='repl',
       MASTER_PASSWORD='Yuanqizhan@163',
       MASTER_AUTO_POSITION=1;
   START SLAVE;
   ```

10. **验证部署**：
    ```bash
    # 检查服务状态
    docker ps | grep hifate
    
    # 检查健康状态
    curl http://8.210.52.217/health
    curl http://47.243.160.43/health
    
    # 检查 MySQL 主从复制
    docker exec -it hifate-mysql-slave mysql -uroot -pYuanqizhan@163 -e "SHOW SLAVE STATUS\G"
    ```

### 日常更新

1. **更新代码**：
   ```bash
   cd /opt/HiFate-bazi
   git pull origin master
   ```

2. **修复 gRPC 代码**（如果更新了 proto）：
   ```bash
   python3 scripts/grpc/fix_version_check.py
   ```

3. **重启服务**：
   ```bash
   cd deploy/docker
   docker-compose -f docker-compose.prod.yml -f docker-compose.node1.yml --env-file /opt/HiFate-bazi/.env restart
   ```

---

## 🔍 运维命令

### 服务状态检查

```bash
# 查看所有服务状态
docker ps | grep hifate

# 查看服务日志
docker logs hifate-web --tail 100
docker logs hifate-bazi-core --tail 100
docker logs hifate-nginx --tail 100

# 检查健康状态
curl http://8.210.52.217/health
curl http://47.243.160.43/health
```

### 数据库维护

```bash
# 检查 MySQL 主从复制状态
docker exec -it hifate-mysql-slave mysql -uroot -pYuanqizhan@163 -e "SHOW SLAVE STATUS\G"

# 检查 MySQL 连接数
docker exec -it hifate-mysql-master mysql -uroot -pYuanqizhan@163 -e "SHOW PROCESSLIST;"

# 检查 Redis 主从复制状态
docker exec -it hifate-redis-slave redis-cli INFO replication
```

### 负载均衡检查

```bash
# 检查 Nginx 配置
docker exec hifate-nginx cat /etc/nginx/conf.d/hifate.conf | grep upstream

# 检查 Nginx 状态
curl http://8.210.52.217/nginx_status

# 检查负载均衡日志
docker logs hifate-nginx --tail 100 | grep upstream
```

---

## ⚠️ 常见问题

### 问题 1: 微服务持续重启

**现象**：
- 微服务（bazi-core, bazi-fortune 等）持续重启
- 错误：`AttributeError: '_Server' object has no attribute 'add_registered_method_handlers'`

**原因**：
- gRPC 版本兼容性问题

**解决方案**：
```bash
# 修复 gRPC 代码
python3 scripts/grpc/fix_version_check.py

# 重启微服务
cd deploy/docker
docker-compose -f docker-compose.prod.yml -f docker-compose.node1.yml --env-file /opt/HiFate-bazi/.env restart
```

### 问题 2: 负载均衡不工作

**现象**：
- 请求只到一个节点
- 或请求失败

**原因**：
- Nginx 配置中的 IP 未替换
- 或使用了公网 IP

**解决方案**：
```bash
# 检查配置
docker exec hifate-nginx cat /etc/nginx/conf.d/hifate.conf | grep upstream

# 如果看到 NODE1_IP 或 NODE2_IP，需要替换
sed -i 's/NODE1_IP/172.18.121.222/g' deploy/nginx/conf.d/hifate.conf
sed -i 's/NODE2_IP/172.18.121.223/g' deploy/nginx/conf.d/hifate.conf
docker restart hifate-nginx
```

### 问题 3: MySQL 主从复制延迟

**现象**：
- `Seconds_Behind_Master > 0`

**解决方案**：
```bash
# 检查从库状态
docker exec -it hifate-mysql-slave mysql -uroot -pYuanqizhan@163 -e "SHOW SLAVE STATUS\G"

# 如果 IO 或 SQL 线程停止，重启从库
docker restart hifate-mysql-slave
```

### 问题 4: 访问慢

**可能原因**：
1. 微服务重启中（等待恢复）
2. 使用公网 IP 跨节点调用（改为内网 IP）
3. Nginx 超时设置过长（已优化）

**检查方法**：
```bash
# 检查服务状态
docker ps | grep Restarting

# 检查 Nginx 配置
docker exec hifate-nginx cat /etc/nginx/conf.d/hifate.conf | grep -E 'NODE1_IP|NODE2_IP|172.18'

# 测试响应时间
time curl http://8.210.52.217/health
```

---

## 📊 性能指标

### 正常指标

| 指标 | 正常值 | 说明 |
|------|--------|------|
| API 响应时间 | < 2秒 | 大多数 API 请求 |
| 服务重启次数 | 0 | 正常运行时不应重启 |
| MySQL 主从延迟 | 0秒 | 完全同步 |
| 负载均衡状态 | 两个节点都正常 | 故障转移正常 |

### 监控命令

```bash
# 检查服务状态
docker ps | grep hifate | grep -E 'Up|Restarting'

# 测试 API 响应时间
time curl -X POST http://8.210.52.217/api/v1/bazi/calculate \
  -H "Content-Type: application/json" \
  -d '{"solar_date":"1990-01-15","solar_time":"12:00","gender":"male"}'

# 检查 MySQL 主从延迟
docker exec -it hifate-mysql-slave mysql -uroot -pYuanqizhan@163 -e "SHOW SLAVE STATUS\G" | grep Seconds_Behind_Master
```

---

## 🔄 更新记录

| 日期 | 更新内容 | 更新人 |
|------|---------|--------|
| 2025-12-12 | 初始版本，记录生产环境架构和部署信息 | AI Assistant |

---

**文档维护**：
- 每次部署后更新状态信息
- 每次架构变更后更新文档
- 每次问题修复后更新常见问题

