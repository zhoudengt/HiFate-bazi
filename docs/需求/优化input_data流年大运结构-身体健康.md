# 优化 input_data 流年大运结构 - 身体健康分析

## 需求背景

参考"八字命理-子女学习"、"八字命理-感情婚姻"和"八字命理-事业财富"的优化方案，对"八字命理-身体健康"的 `input_data` 进行优化，使用方案2（占位符模板），确保大运流年格式与取数规则与其他接口一致。

## 当前问题

### 1. 使用方案1（自然语言 Prompt）
- 代码中包含提示词模板（`build_health_analysis_prompt` 函数）
- 不符合"提示词必须存储在 Coze Bot 中"的要求

### 2. 大运流年格式不一致
- 没有使用 `build_enhanced_dayun_structure` 工具函数
- 没有优先级、描述、备注、人生阶段等信息
- 年龄计算方式与其他接口不一致（使用简单的日期差值计算）
- 与排盘系统不一致
...
3. **优化大运流年提取**：
   - 使用与子女学习/感情婚姻/事业财富一致的逻辑：当前大运 + 关键大运（共10个）
   - 清理流月流日字段
   - 限制每个大运下流年数量为3个（从2个改为3个）

### 阶段三：创建数据构建和格式化函数

1. **优化 `build_health_analysis_input_data` 函数**：
   - 参考 `build_marriage_input_data`、`build_children_study_input_data`、`build_career_wealth_input_data` 的结构
   - 使用新的工具函数
   - 使用引用避免重复
...
1. **修改 `health_analysis_v2_stream_generator` 函数**：
   - 移除 `build_health_analysis_prompt` 调用
   - 使用 `format_input_data_for_coze` 格式化数据
   - 直接发送格式化后的数据给 Bot

2. **创建测试接口**：
   - `POST /api/v1/health-analysis-v2/test` - 返回格式化后的数据
...
1. **创建文档**：`docs/需求/Coze_Bot_System_Prompt_身体健康分析.md`
   - 基于用户提供的提示词
   - 优化和完善提示词
   - 提供可直接复制粘贴的完整版本
...
1. **旺衰数据**：
   - `mingpan_tizhi_zonglun.wangshuai` 不为空

2. **十神数据**：
   - `dayun_jiankang.ten_gods` 不为空

3. **喜忌数据**：
   - `tizhi_tiaoli.xi_ji.xi_shen` 不为空
   - `tizhi_tiaoli.xi_ji.ji_shen` 不为空
   - `tizhi_tiaoli.xi_ji.xi_ji_elements` 不为空

4. **大运流年**：
   - `dayun_jiankang.current_dayun` 不为空（包含优先级、描述、备注、流年等）
   - `dayun_jiankang.key_dayuns` 不为空（至少9个关键大运）
   - `dayun_jiankang.current_dayun.liunians` 每个大运下最多3个流年

## 约束条件

- ✅ **只修改**：`server/api/v1/health_analysis_v2.py`
