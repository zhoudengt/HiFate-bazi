服务器上直接增量部署方法
概述
本计划提供在生产服务器上直接执行增量部署的完整步骤，适用于：

直接在服务器上操作
通过 SSH 远程执行
手动控制部署流程
前置条件
代码已推送到 GitHub (origin/master)
能够 SSH 连接到生产服务器
服务器上项目目录：/opt/HiFate-bazi
方法一：SSH 远程执行（推荐）
步骤 1：连接到 Node1 并拉取代码
# 方式 1：使用 SSH 别名（如果已配置）
ssh hifate-node1 "cd /opt/HiFate-bazi && git pull origin master"

# 方式 2：使用 IP 地址和密钥
ssh -i ~/.ssh/id_rsa_hifate root@8.210.52.217 "cd /opt/HiFate-bazi && git pull origin master"

# 方式 3：使用密码认证
sshpass -p 'Yuanqizhan@163' ssh root@8.210.52.217 "cd /opt/HiFate-bazi && git pull origin master"
步骤 2：连接到 Node2 并拉取代码
# 方式 1：使用 SSH 别名
ssh hifate-node2 "cd /opt/HiFate-bazi && git pull origin master"

# 方式 2：使用 IP 地址和密钥
ssh -i ~/.ssh/id_rsa_hifate root@47.243.160.43 "cd /opt/HiFate-bazi && git pull origin master"

# 方式 3：使用密码认证
sshpass -p 'Yuanqizhan@163' ssh root@47.243.160.43 "cd /opt/HiFate-bazi && git pull origin master"
步骤 3：验证双机代码版本一致
# 获取 Node1 的 Git 版本
NODE1_COMMIT=$(ssh hifate-node1 "cd /opt/HiFate-bazi && git rev-parse HEAD")
echo "Node1: ${NODE1_COMMIT:0:8}"

# 获取 Node2 的 Git 版本
NODE2_COMMIT=$(ssh hifate-node2 "cd /opt/HiFate-bazi && git rev-parse HEAD")
echo "Node2: ${NODE2_COMMIT:0:8}"

# 验证版本一致
if [ "$NODE1_COMMIT" = "$NODE2_COMMIT" ]; then
    echo "✅ 双机代码版本一致"
else
    echo "❌ 双机代码版本不一致，请检查"
    exit 1
fi
步骤 4：触发热更新
# 在 Node1 上触发热更新（会自动同步到 Node2）
curl -X POST http://8.210.52.217:8001/api/v1/hot-reload/sync

# 等待几秒后检查热更新状态
sleep 5
curl http://8.210.52.217:8001/api/v1/hot-reload/status
步骤 5：验证部署结果
# 健康检查
curl http://8.210.52.217:8001/health
curl http://47.243.160.43:8001/health

# 检查 Git 版本
ssh hifate-node1 "cd /opt/HiFate-bazi && git log --oneline -2"
ssh hifate-node2 "cd /opt/HiFate-bazi && git log --oneline -2"
方法二：直接在服务器上执行
步骤 1：SSH 登录到 Node1
# 使用 SSH 别名
ssh hifate-node1

# 或使用 IP 地址
ssh root@8.210.52.217
# 密码：Yuanqizhan@163
步骤 2：在 Node1 上执行部署
# 进入项目目录
cd /opt/HiFate-bazi

# 拉取最新代码
git pull origin master

# 验证代码版本
git log --oneline -2

# 验证语法（可选）
python3 -m py_compile server/api/v1/*.py 2>&1 | head -10
步骤 3：在 Node2 上执行相同操作
# 新开一个终端，SSH 登录到 Node2
ssh hifate-node2

# 进入项目目录
cd /opt/HiFate-bazi

# 拉取最新代码
git pull origin master

# 验证代码版本
git log --oneline -2
步骤 4：验证双机代码一致
在 Node1 和 Node2 上分别执行：

# 获取 Git 版本
git rev-parse HEAD
确保两个服务器返回的 commit hash 完全一致。

步骤 5：触发热更新
在本地或任一服务器上执行：

# 触发 Node1 热更新（会自动同步到 Node2）
curl -X POST http://8.210.52.217:8001/api/v1/hot-reload/sync

# 检查热更新状态
curl http://8.210.52.217:8001/api/v1/hot-reload/status
步骤 6：验证部署
# 健康检查
curl http://8.210.52.217:8001/health
curl http://47.243.160.43:8001/health

# 测试关键接口（可选）
curl -X POST http://8.210.52.217:8001/api/v1/bazi/interface \
  -H "Content-Type: application/json" \
  -d '{"solar_date":"1990-01-15","solar_time":"12:00","gender":"male"}' | jq .
方法三：使用一键部署脚本（在服务器上）
在服务器上创建部署脚本
在 Node1 上创建 /opt/HiFate-bazi/deploy_server.sh：

#!/bin/bash
# 服务器端增量部署脚本

set -e

PROJECT_DIR="/opt/HiFate-bazi"
NODE1_IP="8.210.52.217"
NODE2_IP="47.243.160.43"

echo "🔄 开始增量部署..."

# 1. 在 Node1 上拉取代码
echo "📥 在 Node1 上拉取代码..."
cd $PROJECT_DIR
git pull origin master
NODE1_COMMIT=$(git rev-parse HEAD)
echo "✅ Node1 代码版本: ${NODE1_COMMIT:0:8}"

# 2. 在 Node2 上拉取代码
echo "📥 在 Node2 上拉取代码..."
ssh root@$NODE2_IP "cd $PROJECT_DIR && git pull origin master"
NODE2_COMMIT=$(ssh root@$NODE2_IP "cd $PROJECT_DIR && git rev-parse HEAD")
echo "✅ Node2 代码版本: ${NODE2_COMMIT:0:8}"

# 3. 验证版本一致
if [ "$NODE1_COMMIT" != "$NODE2_COMMIT" ]; then
    echo "❌ 错误：双机代码版本不一致"
    exit 1
fi
echo "✅ 双机代码版本一致"

# 4. 触发热更新
echo "🔄 触发热更新..."
curl -X POST http://$NODE1_IP:8001/api/v1/hot-reload/sync
sleep 5

# 5. 验证部署
echo "🏥 验证部署结果..."
curl -f http://$NODE1_IP:8001/health && echo "✅ Node1 健康"
curl -f http://$NODE2_IP:8001/health && echo "✅ Node2 健康"

echo "✅ 部署完成"
使用脚本
# 在 Node1 上执行
chmod +x /opt/HiFate-bazi/deploy_server.sh
/opt/HiFate-bazi/deploy_server.sh
关键命令速查
Git 操作
# 拉取最新代码
git pull origin master

# 查看当前版本
git rev-parse HEAD

# 查看最近提交
git log --oneline -5

# 检查未提交更改
git status
热更新操作
# 触发热更新
curl -X POST http://8.210.52.217:8001/api/v1/hot-reload/sync

# 检查热更新状态
curl http://8.210.52.217:8001/api/v1/hot-reload/status

# 手动触发检查
curl -X POST http://8.210.52.217:8001/api/v1/hot-reload/check
健康检查
# Node1 健康检查
curl http://8.210.52.217:8001/health

# Node2 健康检查
curl http://47.243.160.43:8001/health
注意事项
双机代码必须一致：部署前确保 Node1 和 Node2 的代码版本完全一致
零停机部署：使用热更新，不会重启服务
验证步骤：部署后必须验证健康检查和关键功能
回滚准备：如果部署失败，可以使用热更新回滚功能
故障排查
问题 1：Git 拉取失败
# 检查网络连接
ping github.com

# 检查 Git 配置
git remote -v

# 手动拉取
cd /opt/HiFate-bazi
git fetch origin
git merge origin/master
问题 2：热更新失败
# 检查服务是否运行
curl http://8.210.52.217:8001/health

# 检查热更新状态
curl http://8.210.52.217:8001/api/v1/hot-reload/status

# 手动触发检查
curl -X POST http://8.210.52.217:8001/api/v1/hot-reload/check
问题 3：双机代码不一致
# 在 Node1 上重置并拉取
cd /opt/HiFate-bazi
git reset --hard origin/master
git pull origin master

# 在 Node2 上重置并拉取
ssh root@47.243.160.43 "cd /opt/HiFate-bazi && git reset --hard origin/master && git pull origin master"