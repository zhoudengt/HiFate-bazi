...
import { storyApi, type StoryLine } from '../../api/story';

const STORY_DIALOGUE_ID = 101;

export default function GanQingDian() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setStoryError(null);
      try {
        const res = await storyApi.getDialogue(STORY_DIALOGUE_ID);
        ...
        if (!cancelled) {
          setStoryError(e instanceof Error ? e.message : '剧情加载失败');
          setStoryLines([]);
        }
      } finally {
  ...
  const showStory = storyLines.length > 0 && storyIndex < storyLines.length;
  const currentLine = showStory ? storyLines[storyIndex] : null;

  const advanceStory = useCallback(() => {
    setStoryIndex((i) => {
      if (i + 1 >= storyLines.length) return storyLines.length;
      return i + 1;
    });
  }, [storyLines.length]);

  const skipStory = useCallback(() => {
    setStoryIndex(storyLines.length);
  }, [storyLines.length]);
    ...
    <div className="relative flex min-h-[100dvh] w-full flex-col overflow-hidden bg-ink">
      ...
      <div className={`relative z-10 flex flex-1 flex-col ${PAGE_BELOW_HUD}`}>
        ...
        >
          <button
            type="button"
            onClick={() => navigate('/liuyao/question?category=love')}
      ...
      {showStory && currentLine ? (
        <DialogueBox
          ...
          onNext={advanceStory}
          onSkip={skipStory}
          side={currentLine.speaker === '玩家自己' ? 'right' : 'left'}
        />
      ) : null}

      {!storyLoading && storyError ? (
        <div className="fixed bottom-0 left-0 right-0 z-40 px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2">
          <p className="rounded-t-xl border border-gold/35 bg-black/75 px-4 py-3 text-center text-sm text-cream/90 backdrop-blur-md">
            {storyError}
          </p>
        </div>
      ) : null}

      <Modal open={placeholderOpen} onClose={() => setPlaceholderOpen(false)} title="牵红线">
