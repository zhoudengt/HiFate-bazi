...
const INTRO_STORAGE_KEY = 'hd2.palace.caishen.intro';

export default function CaiShenDian() {
    ...
    <div className="relative flex min-h-[100dvh] w-full flex-col overflow-hidden bg-ink">
      ...
      <div className={`relative z-10 flex flex-1 flex-col ${PAGE_BELOW_HUD}`}>
        ...
        >
          <button
            type="button"
            onClick={() => navigate('/liuyao/question?category=wealth')}
      ...
      {showIntro ? (
        <DialogueBox
          speaker="财神殿 · 司禄"
          text="善信既至，财气自随。欲问盈亏，可先掷龟壳；或摸福沾财，讨个吉利。"
          onNext={dismissIntro}
          onSkip={dismissIntro}
        />
      ) : null}

      <Modal open={placeholderOpen} onClose={() => setPlaceholderOpen(false)} title="摸福沾财">
