# 🚀 HiFate-bazi 开发者手册

## 一、快速开始（5分钟上手）

### 1.1 环境准备

```bash
# 1. 克隆项目
git clone https://github.com/xxx/HiFate-bazi.git
cd HiFate-bazi

# 2. 创建虚拟环境
python3 -m venv .venv
source .venv/bin/activate  # Mac/Linux
# .venv\Scripts\activate   # Windows

# 3. 安装依赖
pip install -r requirements.txt

# 4. 配置环境变量（复制模板）
cp env.template .env
# 编辑 .env 文件，填写必要配置
```

### 1.2 启动服务

```bash
# 启动主服务（开发模式）
python server/start.py
# 或
./start.sh

# 服务地址
# - Web 服务: http://localhost:8001
# - API 文档: http://localhost:8001/docs
```

### 1.3 验证服务

```bash
# 健康检查
curl http://localhost:8001/api/v1/health

# 测试 API
curl -X POST http://localhost:8001/api/v1/calendar/query \
  -H "Content-Type: application/json" \
  -d '{"date": "2025-12-11"}'
```

---

## 二、开发新功能（30分钟完成）

### 2.1 开发流程图

```
┌─────────────────────────────────────────────────────────────────┐
│                    新功能开发流程                                │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. 创建 API 文件                                               │
│     └─ server/api/v1/{name}_api.py                             │
│                                                                 │
│  2. 创建 Service 文件                                           │
│     └─ server/services/{name}_service.py                       │
│                                                                 │
│  3. 注册 gRPC 端点                                              │
│     └─ server/api/grpc_gateway.py                              │
│                                                                 │
│  4. 注册路由                                                    │
│     └─ server/main.py                                          │
│                                                                 │
│  5. 编写测试                                                    │
│     └─ tests/unit/test_{name}.py                               │
│                                                                 │
│  6. 测试验证                                                    │
│     └─ pytest tests/unit/test_{name}.py                        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 步骤详解

#### 步骤 1：创建 API 文件

**文件位置：** `server/api/v1/{功能名}_api.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
{功能名} API
"""

from fastapi import APIRouter, HTTPException
from pydantic import BaseModel, Field
from typing import Optional, Dict, Any
import logging

from server.services.{功能名}_service import {ClassName}Service

logger = logging.getLogger(__name__)
router = APIRouter()

# ==================== 数据模型 ====================

class {ClassName}Request(BaseModel):
    """请求模型"""
    param1: str = Field(..., description="参数1说明")
    param2: Optional[str] = Field(None, description="参数2说明")

class {ClassName}Response(BaseModel):
    """响应模型"""
    success: bool = Field(..., description="是否成功")
    data: Optional[Dict[str, Any]] = Field(None, description="返回数据")
    error: Optional[str] = Field(None, description="错误信息")

# ==================== API 路由 ====================

@router.post("/{功能路径}/query", response_model={ClassName}Response, summary="功能描述")
async def query_{功能名}(request: {ClassName}Request):
    """
    功能详细说明
    
    Args:
        request: 请求参数
        
    Returns:
        {ClassName}Response: 响应结果
    """
    try:
        service = {ClassName}Service()
        result = service.process(request.param1, request.param2)
        return {ClassName}Response(success=True, data=result)
    except Exception as e:
        logger.error(f"处理失败: {e}", exc_info=True)
        return {ClassName}Response(success=False, error=str(e))
```

#### 步骤 2：创建 Service 文件

**文件位置：** `server/services/{功能名}_service.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
{功能名} 服务
"""

import logging
from typing import Optional, Dict, Any

logger = logging.getLogger(__name__)

class {ClassName}Service:
    """{功能名}服务类"""
    
    def __init__(self):
        """初始化服务"""
        pass
    
    def process(self, param1: str, param2: Optional[str] = None) -> Dict[str, Any]:
        """
        处理业务逻辑
        
        Args:
            param1: 参数1
            param2: 参数2（可选）
            
        Returns:
            处理结果字典
        """
        try:
            # TODO: 实现业务逻辑
            result = {
                "param1": param1,
                "param2": param2
            }
            return result
        except Exception as e:
            logger.error(f"处理失败: {e}", exc_info=True)
            raise
```

#### 步骤 3：注册 gRPC 端点

**文件位置：** `server/api/grpc_gateway.py`

在文件中添加：

```python
# 在 import 部分添加
from server.api.v1.{功能名}_api import {ClassName}Request, query_{功能名}

# 在 SUPPORTED_ENDPOINTS 注册之后添加
@_register("/{功能路径}/query")
async def _handle_{功能名}_query(payload: Dict[str, Any]):
    request_model = {ClassName}Request(**payload)
    return await query_{功能名}(request_model)
```

#### 步骤 4：注册路由

**文件位置：** `server/main.py`

在文件中添加：

```python
# 在 import 部分添加
from server.api.v1.{功能名}_api import router as {功能名}_router

# 在路由注册部分添加
app.include_router({功能名}_router, prefix="/api/v1", tags=["{功能名}"])
logger.info("✓ {功能名}路由已注册")
```

#### 步骤 5：编写测试

**文件位置：** `tests/unit/test_{功能名}.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
{功能名} 单元测试
"""

import pytest
from server.services.{功能名}_service import {ClassName}Service

class Test{ClassName}Service:
    """{ClassName}Service 测试类"""
    
    def setup_method(self):
        """测试前置"""
        self.service = {ClassName}Service()
    
    def test_process_success(self):
        """测试正常处理"""
        # Given
        param1 = "test_value"
        
        # When
        result = self.service.process(param1)
        
        # Then
        assert result is not None
        assert result["param1"] == param1
    
    def test_process_with_optional_param(self):
        """测试带可选参数"""
        # Given
        param1 = "test_value"
        param2 = "optional_value"
        
        # When
        result = self.service.process(param1, param2)
        
        # Then
        assert result["param2"] == param2
```

#### 步骤 6：运行测试

```bash
# 运行单个测试
pytest tests/unit/test_{功能名}.py -v

# 运行所有测试
pytest tests/ -v

# 查看覆盖率
pytest tests/ --cov=server --cov-report=html
```

---

## 三、代码生成器（推荐）

使用代码生成器快速创建新功能：

```bash
# 生成新功能（API + Service + Test）
python scripts/generate.py api --name=feature_name --desc="功能描述"

# 示例
python scripts/generate.py api --name=calendar --desc="万年历查询"
```

生成的文件：
```
✅ server/api/v1/calendar_api.py
✅ server/services/calendar_service.py
✅ tests/unit/test_calendar.py
📝 请手动在 grpc_gateway.py 和 main.py 中注册路由
```

---

## 四、开发规范速查

### 4.1 文件命名

| 类型 | 格式 | 示例 |
|------|------|------|
| API | `{name}_api.py` | `calendar_api.py` |
| Service | `{name}_service.py` | `calendar_service.py` |
| 测试 | `test_{name}.py` | `test_calendar.py` |

### 4.2 类命名

| 类型 | 格式 | 示例 |
|------|------|------|
| 请求模型 | `{Name}Request` | `CalendarRequest` |
| 响应模型 | `{Name}Response` | `CalendarResponse` |
| 服务类 | `{Name}Service` | `CalendarService` |

### 4.3 API 路径

| 规范 | 示例 |
|------|------|
| 小写+斜杠 | `/calendar/query` |
| 动作+名词 | `/bazi/calculate` |

---

## 五、常用命令

### 5.1 开发命令

```bash
# 启动服务
./start.sh

# 停止服务
./stop.sh

# 查看日志
tail -f logs/server_8001.log

# 运行测试
pytest tests/ -v

# 检查代码风格
flake8 server/ --max-line-length=120
```

### 5.2 Git 命令

```bash
# 创建功能分支
git checkout -b feature/calendar-api

# 提交代码
git add .
git commit -m "[新增] 万年历查询功能"

# 推送分支
git push origin feature/calendar-api
```

### 5.3 数据库命令

```bash
# 连接数据库
mysql -h localhost -u root -p hifate_bazi

# 运行迁移
python scripts/migration/run_migration.py
```

---

## 六、调试技巧

### 6.1 查看请求日志

```bash
# 查看最新日志
tail -f logs/server_8001.log

# 搜索特定关键词
grep "calendar" logs/server_8001.log
```

### 6.2 API 调试

```bash
# 使用 curl 测试
curl -X POST http://localhost:8001/api/v1/calendar/query \
  -H "Content-Type: application/json" \
  -d '{"date": "2025-12-11"}'

# 使用 Swagger UI
# 访问 http://localhost:8001/docs
```

### 6.3 断点调试

```python
# 在代码中添加断点
import pdb; pdb.set_trace()

# 或使用 IDE 的调试功能
```

---

## 七、常见问题

### Q1: 服务启动失败

```bash
# 检查端口占用
lsof -i :8001

# 杀死占用进程
kill -9 <PID>
```

### Q2: 模块导入失败

```bash
# 确保在项目根目录
cd /path/to/HiFate-bazi

# 激活虚拟环境
source .venv/bin/activate

# 重新安装依赖
pip install -r requirements.txt
```

### Q3: 数据库连接失败

```bash
# 检查 .env 配置
cat .env | grep MYSQL

# 测试连接
mysql -h localhost -u root -p
```

---

## 八、开发检查清单

新功能开发完成前，检查以下事项：

- [ ] API 文件已创建（`server/api/v1/`）
- [ ] Service 文件已创建（`server/services/`）
- [ ] gRPC 端点已注册（`grpc_gateway.py`）
- [ ] 路由已注册（`main.py`）
- [ ] 单元测试已编写（`tests/unit/`）
- [ ] 测试全部通过
- [ ] 代码风格检查通过
- [ ] 文档已更新（如需要）

---

## 九、获取帮助

- 📖 详细文档：`docs/` 目录
- 📋 开发规范：`standards/` 目录
- 💬 技术支持：联系项目负责人
